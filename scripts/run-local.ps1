param(
    [switch]$SetupPin
)

$ErrorActionPreference = "Stop"

$projectDir = Split-Path -Parent $PSScriptRoot
Set-Location $projectDir

if (-not (Get-Command uv -ErrorAction SilentlyContinue)) {
    throw "Ghost Lap needs uv. Install it from https://docs.astral.sh/uv/getting-started/installation/ and run this launcher again; uv installs compatible Python if needed and downloads the locked dependencies."
}

$env:HITL2_MODE = "hardware"
if (-not $env:HITL2_HOST) { $env:HITL2_HOST = "localhost" }
if (-not $env:HITL2_PORT) { $env:HITL2_PORT = "8788" }
if (-not $env:HITL2_OPEN_BROWSER) { $env:HITL2_OPEN_BROWSER = "1" }
if ($SetupPin) {
    $env:HITL2_ALLOW_INITIAL_PIN = "1"
    Write-Warning "One-time PIN setup can change the sole connected FIDO key. Leave only the intended key connected. Existing PINs are never changed."
} else {
    Remove-Item Env:HITL2_ALLOW_INITIAL_PIN -ErrorAction SilentlyContinue
}

$identity = [Security.Principal.WindowsIdentity]::GetCurrent()
$principal = [Security.Principal.WindowsPrincipal]::new($identity)
$administrator = [Security.Principal.WindowsBuiltInRole]::Administrator
$exitCode = 0
try {
    if ($principal.IsInRole($administrator)) {
        if (-not (Test-Path (Join-Path $projectDir ".venv"))) {
            throw "Install dependencies once from a non-Administrator PowerShell first: uv sync --locked"
        }
        Write-Warning "Elevated mode expands the local broker's impact. Use it only on a trusted single-user machine, then stop it after the demo."
        uv run --offline --no-sync hitl2
        $exitCode = $LASTEXITCODE
    } else {
        Write-Warning "Direct FIDO USB access is restricted on current Windows versions. If the key is not detected, reopen PowerShell as Administrator."
        uv sync --locked
        $exitCode = $LASTEXITCODE
        if ($exitCode -eq 0) {
            uv run --no-sync hitl2
            $exitCode = $LASTEXITCODE
        }
    }
} finally {
    if ($SetupPin) {
        Remove-Item Env:HITL2_ALLOW_INITIAL_PIN -ErrorAction SilentlyContinue
    }
}
exit $exitCode
